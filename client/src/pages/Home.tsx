...
export default function Home() {
  ...
  useEffect(() => {
    const updateGlobeSize = () => {
      setGlobeSize(window.innerWidth < 768 ? 240 : 320);
    };
    updateGlobeSize();
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-gray-100 font-sans selection:bg-blue-200/50 dark:selection:bg-blue-900/50 selection:text-blue-900 dark:selection:text-blue-100">
      ...
      <section className="max-w-7xl mx-auto px-6 pb-20">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          {/* Geospatial with responsive globe */}
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 grid-flow-dense">
          {tools.map((tool, idx) => {
            ...
            // Responsive col-span logic
            const baseSpan = tool.size?.includes('col-span-3')
              ? 'col-span-1 lg:col-span-3'
              : tool.size?.includes('col-span-2')
              ? 'col-span-1 lg:col-span-2'
              : 'col-span-1';

            const isLarge = tool.size?.includes('col-span-2') || tool.size?.includes('col-span-3');

            return (
              <Link key={idx} href={tool.href} className={baseSpan}>
                <a
                ...
                >
                  ...
                  {/* Hover glow */}
                  <div
                    className={`absolute -right-4 -bottom-4 w-40 h-40 rounded-full bg-gradient-to-br ${colors.light.split(' ')[0]} dark:${colors.dark.split(' ')[0]} blur-3xl opacity-0 group-hover:opacity-40 transition-opacity duration-700`}
                  />
                </a>
              </Link>
            );
