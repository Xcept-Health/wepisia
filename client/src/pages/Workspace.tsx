/**
 * EpidemiologicalSimulation.tsx — Production v3
 *
 * Bug fixes vs v1:
 *  - models object moved outside component (no re-creation each render)
 *  - Map/Globe completely separated from simulation loop → zero flickering
 *  - computeTrajectory() is pure and precomputed → no blocking setInterval
 *  - All modals use controlled state (no getElementById)
 *  - Globe WebGL cleanup: renderer.dispose() + forceContextLoss()
 *  - Leaflet: separate init / data-update effects
 *  - indicator.color no longer undefined
 *  - History bounded to simulation days
 *  - TypeScript types throughout (no any in hot paths)
 *
 * New features:
 *  - Timeline slider: scrub J0–J365 on the map
 *  - Rt (effective reproduction number) chart
 *  - Daily new cases chart
 *  - Phase portrait (S vs I trajectory)
 *  - Dark / light theme via project CSS variables
 *  - Mobile bottom navigation (4 tabs, full-screen panels)
 *  - Responsive map, charts, network
 */
